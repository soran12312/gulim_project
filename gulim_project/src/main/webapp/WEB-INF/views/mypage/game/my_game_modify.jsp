...
$(function(){
	var tagList = '<c:out value="${tagList}" />';
	console.log(tagList);
			...
			<table class = "createRoomtable">
				<tr>
					...
					<td class="room_videocheck">
						<div>화상채팅</div>
						<c:if test="${room.video == 0}">
							<span>필수</span>
							<input type="radio" name="video" value="0" checked="checked">
							<span>선택</span>
							<input type="radio" name="video" value="1">
						</c:if>
						<c:if test="${room.video == 1}">
							<span>필수</span>
							<input type="radio" name="video" value="0">
							<span>선택</span>
							<input type="radio" name="video" value="1" checked="checked">
						</c:if>
				...
				<tr>
					...
					<td class="creat_period">
						<div>예상 플레이 기간</div>
						<input type="number" min="1" name="period" value="${room.period}">주
					</td>
				</tr>
				<tr>
				...
				<tr>
					<td colspan="3"><textarea name="plot" placeholder="줄거리를 써 주세요" rows="4" cols="80">${room.plot}</textarea></td>
				</tr>
				<tr>
					<td colspan="3">시나리오 규칙</td>
				</tr>
				<tr>
					<td colspan="3"><textarea name="law" placeholder="규칙를 써 주세요" rows="4" cols="80">${room.law}</textarea></td>
				</tr>
				<tr>
					<td colspan="3">기타사항</td>
				</tr>
				<tr>
					<td colspan="3"><textarea name="etc" placeholder="기타사항을 써 주세요" rows="4" cols="80">${room.etc}</textarea></td>
				</tr>
				<tr>
					<td>
						<button type="submit" class="creatRoom_btn">방 수정하기</button>
					</td>
				</tr>
			</table>
		</form>
